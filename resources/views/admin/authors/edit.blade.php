    ...
    <div class="container fs-normal">
        ...
        <div class="card shadow mb-4 border-0">
            ...
            <div class="card-body container-fluid">
                <form method="post" action="/u/author/{{ $responseById['id'] }}">
                    @method('PUT')
                    @csrf
                    <div class="form-group">
                        ...
                        <input type="text" data-id="inputNameBrand" placeholder="Nama Brand"
                            ...
                            value="{{ $responseById['name'] }}" autofocus>
                        @error('name')
                            <div class="invalid-feedback ml-1">Bidang ini wajib diisi</div>
                        @enderror
                    </div>

                    <input type="hidden" id="brandId" name="brandId" value="{{ $responseById['id'] }}">
                    ...
                    <div class="form-group">
                        ...
                        <select id="category" name="category" placeholder="Kategori Brand"
                            ...
                            <option value="1">Sunglasses
                            </option>
                            <option value="2">Makeup</option>
                            {{-- <option value="1" {{ $responseById['category'] == '1' ? 'selected' : '' }}>Sunglasses
                            </option>
                            <option value="2" {{ $responseById['category'] == '2' ? 'selected' : '' }}>Makeup</option> --}}
                        </select>
                        @error('category')
                    ...
                    <div class="form-group">
                        <label for="photo_brand">Photo Brand</label>
                        <input type="file"
                            class="form-control fs-normal border-0 @error('photo_brand') is-invalid @enderror"
                            id="photo_brand" name="photo_brand" data-toggle="tooltip" data-placement="right"
                            value="{{ $responseById['image'] }}">
                        @error('photo_brand')
                            <div class="invalid-feedback ml-1">{{ $message }}</div>
                        @enderror
                    </div>

                    <button type="submit" data-id="btnUpdateAuthor"
                        class="btn btn-primary font-medium float-right py-2 px-5">Update</button>
                </form>
            </div>
